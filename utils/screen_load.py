...
def create_load_screen(mss: ModelSessionState):
    ...
    if mss.source_relation is None:
        mss.source_relation = Relation(name="New source")
    if mss.target_relation is None:
        mss.target_relation = Relation(name="New target")
...
def _display_relation(
    ...
    def on_change_relname(relation: Relation, session_key: str):
    ...
    rel_key_name = f"{side}_relation_name"
    st.text_input(
        "Relation name",
        value=relation.name,
        key=rel_key_name,
        ...
        with st.expander(f"Attribute {i+1} details"):
            ...
            if result is not None and attr.uid is not None:
                ...
                st.text_area(
                    ...
                    disabled=True,
                    key=f"{relation.name}.Attr{attribute.uid}.description.result",
                )
            if compare_to is not None and attr.uid is not None:
                ...
                st.text_area(
                    ...
                    value=attribute.description,
                    disabled=True,
                    key=f"{relation.name}.Attr{attribute.uid}.description.compare_to",
                )
