...
def create_visualize_screen(mss: ModelSessionState):
    ...
    cols = st.columns(6)
    with cols[0]:
        show_yes = st.checkbox("Yes", value=True, key="vote_yes_checkbox")
    with cols[1]:
        show_no = st.checkbox("No", value=True, key="vote_no_checkbox")
    with cols[2]:
        show_unknown = st.checkbox("Unknown", value=True, key="vote_unknown_checkbox")
